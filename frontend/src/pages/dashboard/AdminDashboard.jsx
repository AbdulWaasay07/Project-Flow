...
import {
  ...
  UserCheck, Briefcase, TrendingUp, Clock, Calendar,
  ChevronDown, Target, Activity
} from 'lucide-react';
...
export default function AdminDashboard() {
  ...
  const loadDashboard = async () => {
    try {
      ...
      if (progRes.status === 'fulfilled')    setProjectsProgress(progRes.value || []);
  ...
  const openStats = async (member) => {
    ...
    try {
      ...
      // Backend endpoint may not exist — build from workload data
      setStatsData({
        totalProjects:  null,
        totalTasks:     member.taskCount      ?? null,
        completedTasks: member.completedTasks ?? null,
        overdueTasks:   member.overdueTasks   ?? null,
        overallCompletionPercentage: member.taskCount && member.completedTasks
  ...
  return (
    <div>
      ...
      <div className="page-header">
        ...
        <p className="page-subtitle">System-wide overview — all projects, members &amp; tasks.</p>
      </div>
        ...
        {kpis.map((k) => (
          <div key={k.label} className="stat-card">
            <div className="stat-card-header">
              ...
              {/* Inline colored icon — avoids any broken CSS class */}
              <div style={{
      ...
      {/* ── 2-col grid: Project Progress + Upcoming Deadlines ── */}
          ...
          <div className="card-content">
            {projectsProgress.length === 0 ? (
              <div style={{ textAlign: 'center', padding: '2rem', color: 'var(--color-text-secondary)' }}>
                <FolderKanban size={36} style={{ opacity: 0.2, marginBottom: '0.5rem' }} />
                <p>No projects yet</p>
              </div>
            ) : (
                  ...
                  return (
                    <div key={p.projectId} style={{
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '0.45rem' }}>
                        {p.managerName && (
                          <span style={{ fontSize: '0.72rem', color: 'var(--color-text-secondary)', display: 'flex', alignItems: 'center', gap: '0.25rem' }}>
                            <UserCheck size={11} style={{ color: '#6366f1' }} /> {p.managerName}
                          </span>
                        )}
                        <span style={{ fontSize: '0.72rem', color: 'var(--color-text-secondary)', display: 'flex', alignItems: 'center', gap: '0.25rem' }}>
                          <CheckSquare size={11} /> {p.completedTasks}/{p.totalTasks}
                        </span>
                      </div>
                    ...
                    >
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', marginBottom: '0.15rem' }}>
                          ...
                          <span style={{ fontSize: '0.6rem', fontWeight: 700, padding: '0.1rem 0.38rem', borderRadius: '99px', background: sts.bg, color: sts.color, whiteSpace: 'nowrap', flexShrink: 0 }}>
                            {sts.label}
                          </span>
                        </div>
                        <span style={{ fontSize: '0.7rem', color: 'var(--color-text-secondary)', display: 'flex', alignItems: 'center', gap: '0.25rem' }}>
                          ...
                          {p.managerName && <span style={{ marginLeft: '0.5rem', display: 'flex', alignItems: 'center', gap: '0.2rem' }}><UserCheck size={10} style={{ color: '#6366f1' }} /> {p.managerName}</span>}
                        </span>
                      </div>
        ...
        <div className="card" style={{ marginTop: '1.5rem' }}>
          <div className="card-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <h2 className="card-title" style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
              <TrendingUp size={19} style={{ color: '#6366f1' }} /> Team Workload
            </h2>
            <span style={{ fontSize: '0.75rem', color: 'var(--color-text-secondary)' }}>
              {stats.teamWorkload.length} members · click to view stats
            </span>
          </div>
          <div className="card-content">
            {(() => {
              const sorted  = [...stats.teamWorkload].sort((a, b) => b.taskCount - a.taskCount);
              const maxVal  = sorted[0]?.taskCount || 1;
              return (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.6rem' }}>
                  {sorted.map((member, i) => {
                    const barPct  = Math.round((member.taskCount / maxVal) * 100);
                    const avatarC = AVATAR_COLORS[i % AVATAR_COLORS.length];
                    const level   = member.taskCount === 0 ? 'idle'
                      : member.taskCount >= maxVal * 0.75 ? 'high' : 'normal';
                    const levelColor = level === 'high' ? '#ef4444' : level === 'idle' ? '#9ca3af' : '#6366f1';
                    const levelBg    = level === 'high' ? 'rgba(239,68,68,0.1)' : level === 'idle' ? 'rgba(156,163,175,0.1)' : 'rgba(99,102,241,0.1)';
                    ...
                    return (
                      ...
                      >
                        <div style={{ width: '36px', height: '36px', borderRadius: '50%', background: avatarC, color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, fontSize: '0.85rem', flexShrink: 0 }}>
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.3rem' }}>
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', flexShrink: 0 }}>
                              <span style={{ fontSize: '0.68rem', fontWeight: 700, padding: '0.1rem 0.4rem', borderRadius: '99px', background: levelBg, color: levelColor }}>
                                {member.taskCount} task{member.taskCount !== 1 ? 's' : ''}
                              </span>
                            </div>
                          </div>
                          <div style={{ height: '5px', borderRadius: '99px', background: 'var(--color-neutral)', overflow: 'hidden' }}>
                            <div style={{ height: '100%', width: `${barPct}%`, borderRadius: '99px', background: level === 'high' ? '#ef4444' : avatarC, transition: 'width 0.6s ease' }} />
                          </div>
                        </div>
                        <Briefcase size={15} style={{ color: 'var(--color-text-secondary)', flexShrink: 0 }} />
                      </div>
                    );
                  })}
                </div>
        ...
        {statsLoading ? (
          ...
          <div style={{ padding: '0.5rem 0' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', padding: '1rem', background: 'var(--color-surface-raised)', borderRadius: 'var(--radius-md)', marginBottom: '1.25rem', border: '1px solid var(--color-neutral)' }}>
              <div style={{ width: '52px', height: '52px', borderRadius: '50%', background: 'linear-gradient(135deg, #6366f1, #8b5cf6)', color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, fontSize: '1.3rem', flexShrink: 0 }}>
                {statsTarget.userName?.charAt(0)}
              </div>
              <div>
                <p style={{ fontWeight: 700, fontSize: '1.05rem' }}>{statsTarget.userName}</p>
                <p style={{ fontSize: '0.78rem', color: 'var(--color-text-secondary)' }}>Team Member</p>
              </div>
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.7rem', marginBottom: '1.25rem' }}>
              {[
                ...
                <div key={item.label} style={{
                  padding: '0.85rem', background: 'var(--color-surface-raised)',
                  borderRadius: 'var(--radius-md)', border: '1px solid var(--color-neutral)',
            ...
            {/* Completion bar — only if data available */}
            {statsData && (statsData.overallCompletionPercentage != null || (statsData.totalTasks > 0 && statsData.completedTasks != null)) ? (() => {
              ...
              return (
                <div style={{ padding: '1rem', background: 'var(--color-surface-raised)', borderRadius: 'var(--radius-md)', border: '1px solid var(--color-neutral)' }}>
                  ...
                  <div style={{ height: '9px', borderRadius: '99px', background: 'var(--color-neutral)', overflow: 'hidden' }}>
              ...
              <p style={{ fontSize: '0.8rem', color: 'var(--color-text-secondary)', textAlign: 'center', padding: '0.75rem', background: 'var(--color-surface-raised)', borderRadius: 'var(--radius-md)', border: '1px solid var(--color-neutral)' }}>
                Detailed stats are not yet available for this member.
              </p>
            )}
          </div>
        )}
